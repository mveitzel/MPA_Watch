...
setwd("/Users/lilyschneekluth/Desktop/MPA WATCH/MPAWATCH")

#dat<-read.csv("CleanedMPAWatchInterceptSurveyData_09252024-FullDataset.csv",skip=1)
dat<-read.csv("CleanedMPAWatchInterceptSurveyData_09252024_edited.csv",skip=1,sep="\t")
#dat<-read.csv("mpawatchdata.csv.numbers",skip=1,sep="\t")
...
#plot with specific color for each group
#ggplot(df,aes(x, y, color = group)) + geom_point() + scale_color_manual(values = c("
#Aware of CA MPAs"="1c428a","Unaware of CA MPAs" = "71c5e9"))

mpaa<-ggplot(mpaaware,
             df,aes(x=Var1, y=Freq, fill=Var2, color = group)) + geom_point() + scale_color_manual(values = c("
Aware of CA MPAs"="1c428a","Unaware of CA MPAs" = "71c5e9"))

mpaa<-ggplot(mpaaware,aes(x=Var1,y=Freq, fill=Var2))+
